...
on run {input, parameters}
	...
	set checkSum to the text returned of (display dialog "Paste MD5Sum" default answer "" with title fileName)
	
	-- Calculate MD5 sum checksum quitely and translate to UPPER case
	set checkSumResult to first word of (do shell script "/sbin/md5 -q " & quoted form of POSIX path of chosenFile & " | tr '[:lower:]' '[:upper:]'")
	...
	if checkSumComp = true then
		display dialog displayText & "MD5um OK!" buttons {"OK"} default button "OK" with title fileName
	else
		display dialog displayText & "NB! No MD5Sum match!" buttons {"CANCEL"} default button "CANCEL" with title fileName with icon caution
	end if
